...
sim_analyse <- function(condition, dat, fixed_objects = NULL) {
  ...
  dens_temp_bmlvar  <- NULL
  dens_cont_bmlvar  <- NULL
  instrength_bmlvar  <- NULL
  outstrength_bmlvar <- NULL
  strength_bmlvar    <- NULL
  outstrength_bmlvar_first <- NULL
  strength_bmlvar_first    <- NULL
  instrength_bmlvar_first  <- NULL
  ...
  rhat_bmlvar_tmp  <- NULL
  rhat_bmlvar_tmp  <- NULL
  rhat_bmlvar_mean <- NULL
  rhat_bmlvar_11   <- NULL
  divtrans_bmlvar  <- NULL
...
sim_results_rev2_no_bayes <- SimDesign::runSimulation(
  ...
  packages     = c("tidyverse", "gimme", "mlVAR", "graphicalVAR",
                   ...
                   "rstan", "corpcor", "Rcpp"),
  save_results = TRUE,
  filename     = "sim_full_rev2_additional_no_bayes.rds"
)
...
#' 
## ----eval = FALSE-------------------------------------------------------------------------------------------------------------------------------------
# knitr::purl(here::here("scripts", "09_additional_simulations_no_bayes.qmd"),
#             output = here::here("scripts", "09_additional_simulations_no_bayes.R"),
#             documentation = 2)
