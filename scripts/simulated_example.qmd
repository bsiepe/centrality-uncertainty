...
# Combine data to data.frame
Y <- do.call(rbind, ml_data)
mlvar_res <- mlVAR::mlVAR(Y,
                          vars = c("V1", "V2", "V3", "V4", "V5", "V6"),
                          idvar = "ID")

# Get individual network
net_1 <- mlVAR::getNet(mlvar_res, 
                       subject = 1,
                       type = "temporal")
...
```{r}
# graphicalVAR
emp_centrality_gvar <- lapply(ml_fit, centrality_graphicalvar)

# GIMME
emp_centrality_gimme <- centrality_gimme(gimme_fit)
...
tempdens_emp_gvar <- unlist(lapply(emp_centrality_gvar, function(x) x$dens_temp))
tempdens_emp_gimme <- unlist(emp_centrality_gimme$dens_temp)
reg_data <- reg_data %>%
  mutate(
