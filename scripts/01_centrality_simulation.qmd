...
n_sim_sd <- 10000
sd_results <- vector("list", length = nrow(df_design))
...
saveRDS(sd_results, here::here("data", "true_strength_sd.RDS"))

```

As we only need to compute the true SD once, we can simply load it from disk to save time: 
```{r}
true_strength_sd <- readRDS(here::here("data", "true_strength_sd.RDS"))
df_design$strength_sd <- true_strength_sd
```
