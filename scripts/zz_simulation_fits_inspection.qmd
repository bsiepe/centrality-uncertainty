...
for(i in 1:10){
  l_coefs$zero[i] <- summary(r1$results[[i]]$gvar$reg_outstrength[[1]])$coefficients[2]
  l_coefs$two[i] <- summary(r1$results[[i]]$gvar$reg_outstrength[[2]])$coefficients[2]
  l_coefs$four[i] <- summary(r1$results[[i]]$gvar$reg_outstrength[[3]])$coefficients[2]
}
...
df_centrality_mlvar |> 
  ...
  pivot_wider(id_cols = c(sim_rep, est, centrality), names_from = summary, values_from = value) |> View()
```
