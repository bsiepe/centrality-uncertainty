...
```{r}
param_ests <- summary(mlvar_fit)$summary %>% as.data.frame()
```
...
```{r}
fit <- mlvar_fit
list <- extract_all_estimates(mlvar_fit, I, K)
```

```{r}
beta_est <- extract_estimates(fit, "Beta") %>% 
  map(function(x) est_vector2matrix(x, I, K))
pcor_est <-  extract_estimates(fit, "Rho") %>% 
  map(function(x) est_vector2matrix(x, I, K))
draws <-
    rstan::extract(fit, "Rho_vec", permute = FALSE) %>%
    as_draws_matrix()
  # calculate estimates
  median <-
    try(bayestestR::point_estimate(draws)[, "Median"])
  di_95_L <- try(bayestestR::hdi(draws, ci = .95)[, "CI_low"])
  hdi_95_U <- try(bayestestR::hdi(draws, ci = .95)[, "CI_high"])
  q_05 <- try(bayestestR::ci(draws, ci = .90)[, "CI_low"])
...
pcor_centrality_est <-  extract_estimates(fit, "Rho_centrality") %>% 
  map(function(x) est_vector2vector(x, I, K))

contdens_est <-  extract_estimates(fit, "Rho_density")
tempdens_est <-  extract_estimates(fit, "Beta_density")
outstrength_est <-  extract_estimates(fit, "Beta_out_strength")

regression_slope_est <- extract_estimates(fit, "reg_slope_density")
regression_intercept_est <- extract_estimates(fit, "reg_intercept")
```
