...
# Run sampler
mlvar_fit <- var_lkj_model$variational(
  data = stan_data,
  seed = 2023,
  init = 0,
  adapt_engaged = FALSE,
...
```{r}
#param_ests <- mlvar_fit$summary(c("B", "Rho","mu_B","sigma_B", "sigma_theta"))
...
bayesplot::mcmc_recover_intervals(
  true = .25,
  x = mlvar_fit$draws("reg_slope_density") %>% 
    as_draws_matrix())
