...
stan_data <-
  list(
    ...
    idx_rho = idx_rho,
    Y = Y,
    reg_covariate = reg_data$depression
  )
# Choose model to fit
model_name <- "MLVAR"
# Compile model
...
extract_stan_median_vec <- function(fit, par_name, n_var, n_person) {
  ...
  medians <- rstan::summary(fit, pars =  par_name, probs = .5)[,3] %>% 
    unlist()
...
bayesplot::mcmc_recover_intervals(
  true = c(0,.25, log(eps)),
  x = rstan::extract(object = mlvar_fit, c("reg_intercept", "reg_slope_density", "reg_residual")) %>% 
    as_draws_matrix())
```
...
# plot
plot(Beta_density, Beta_density_true, xlim = c(0, .5), ylim = c(0, .5))
abline(a = 0, b = 1, col = "red")
```
...
# plot
plot(Rho_density, Rho_density_true, xlim = c(0, .5), ylim = c(0, .5))
abline(a = 0, b = 1, col = "red")
```
...
# plot
plot(in_strength, in_strength_true, xlim = c(0, .5), ylim = c(0, .5))
abline(a = 0, b = 1, col = "red")
```
...
# plot
plot(out_strength, out_strength_true, xlim = c(0, .5), ylim = c(0, .5))
abline(a = 0, b = 1, col = "red")
```
...
# plot
plot(Rho_strength, Rho_strength_true, xlim = c(0, .5), ylim = c(0, .5))
abline(a = 0, b = 1, col = "red")
```
