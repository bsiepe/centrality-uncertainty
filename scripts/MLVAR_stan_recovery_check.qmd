...
ml_outstrength <- lapply(ml_beta, function(x){colMeans(abs(x))})
ml_instrength <- lapply(ml_beta, function(x){rowMeans(abs(x))})
ml_strength <- lapply(ml_pcor, function(x){
  diag(x) <- 0
  x[lower.tri(x)] <- 0L
  colMeans(abs(x))
})
ml_temporal_density <- lapply(ml_beta, function(x)mean(abs(x)))
...
bayesplot::mcmc_recover_intervals(
  true = c(0,.25),
  x = mlvar_fit$draws(c("reg_intercept", "reg_slope_density")) %>% 
    as_draws_matrix())
```
...
# plot
plot(Beta_density, Beta_density_true)
abline(a = 0, b = 1, col = "red")
```
...
# plot
plot(Rho_density, Rho_density_true)
abline(a = 0, b = 1, col = "red")
```
...
# plot
plot(in_strength, in_strength_true)
abline(a = 0, b = 1, col = "red")
```
...
# plot
plot(out_strength, out_strength_true)
abline(a = 0, b = 1, col = "red")
```
...
# plot
plot(Rho_strength, Rho_strength_true)
abline(a = 0, b = 1, col = "red")
```
