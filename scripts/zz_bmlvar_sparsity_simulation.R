...
sim_analyse <- function(condition, dat, fixed_objects = NULL){
  ...
  fit_bmlvar_sparse1 <- rstan::sampling(
  ...
  if(!inherits(fit_bmlvar_sparse1, "stanfit")){
    stop("BmlVAR model failed.")
  }
  
  stan_data$sparsity <- 2
  
  fit_bmlvar_sparse2 <- rstan::sampling(
  object = fixed_objects$mlvar_model,
  pars = vars(contains("raw")),
  include = FALSE,
  data = stan_data,
  ...
  if(!inherits(fit_bmlvar_sparse2, "stanfit")){
    stop("BmlVAR model failed.")
  }
  
  stan_data$sparsity <- 3
  
  fit_bmlvar_sparse3 <- rstan::sampling(
  object = fixed_objects$mlvar_model,
  pars = vars(contains("raw")),
  include = FALSE,
  data = stan_data,
  ...
  if(!inherits(fit_bmlvar_sparse3, "stanfit")){
    stop("BmlVAR model failed.")
  }
  
  # Extract estimates
  ests_bmlvar_sparse1 <- extract_all_estimates(fit_bmlvar_sparse1, n_id = n_id, n_var = n_var)
  ests_bmlvar_sparse2 <- extract_all_estimates(fit_bmlvar_sparse2, n_id = n_id, n_var = n_var)
  ests_bmlvar_sparse3 <- extract_all_estimates(fit_bmlvar_sparse3, n_id = n_id, n_var = n_var)
  
  # Obtain centralities
  dens_temp_bmlvar_sparse1 <- ests_bmlvar_sparse1$tempdens_est$median
  dens_temp_bmlvar_sparse2 <- ests_bmlvar_sparse2$tempdens_est$median
  dens_temp_bmlvar_sparse3 <- ests_bmlvar_sparse3$tempdens_est$median
  
  dens_cont_bmlvar_sparse1 <- ests_bmlvar_sparse1$contdens_est$median
  dens_cont_bmlvar_sparse2 <- ests_bmlvar_sparse2$contdens_est$median
  dens_cont_bmlvar_sparse3 <- ests_bmlvar_sparse3$contdens_est$median
  
  outstrength_bmlvar_sparse1 <- ests_bmlvar_sparse1$outstrength_est$median
  outstrength_bmlvar_sparse2 <- ests_bmlvar_sparse2$outstrength_est$median
  outstrength_bmlvar_sparse3 <- ests_bmlvar_sparse3$outstrength_est$median
  
  strength_bmlvar_sparse1 <- ests_bmlvar_sparse1$pcor_centrality_est$median
  strength_bmlvar_sparse2 <- ests_bmlvar_sparse2$pcor_centrality_est$median
  strength_bmlvar_sparse3 <- ests_bmlvar_sparse3$pcor_centrality_est$median
  
  outstrength_bmlvar_first_sparse1 <- sapply(outstrength_bmlvar_sparse1, function(x) unname(x[1]))
  outstrength_bmlvar_first_sparse2 <- sapply(outstrength_bmlvar_sparse2, function(x) unname(x[1]))
  outstrength_bmlvar_first_sparse3 <- sapply(outstrength_bmlvar_sparse3, function(x) unname(x[1]))

  # Obtain regression coefficients
  reg_bmlvar_sparse1 <- list(
    regression_intercept = ests_bmlvar_sparse1$regression_intercept_est, 
    regression_slope = ests_bmlvar_sparse1$regression_slope_est)
  
  reg_bmlvar_sparse2 <- list(
    regression_intercept = ests_bmlvar_sparse2$regression_intercept_est, 
    regression_slope = ests_bmlvar_sparse2$regression_slope_est)
  
  reg_bmlvar_sparse3 <- list(
    regression_intercept = ests_bmlvar_sparse3$regression_intercept_est, 
    regression_slope = ests_bmlvar_sparse3$regression_slope_est)
  
  # Save summaries 
  summary_bmlvar_sparse1 <- rstan::summary(fit_bmlvar_sparse1)
  summary_bmlvar_sparse2 <- rstan::summary(fit_bmlvar_sparse2)
  ...
  divtrans_bmlvar_sparse3 <- rstan::get_num_divergent(fit_bmlvar_sparse3)
  
  #--- Return Results
  # Also return true centralities for comparison later
  ret_results <- list(
    bmlvar_sparse1 = list(
      fit_bmlvar_sparse1 = list(
        beta = ests_bmlvar_sparse1$beta_est$median,
        pcor = ests_bmlvar_sparse1$pcor_est$median
      ),
      dens_temp = dens_temp_bmlvar_sparse1,
      dens_cont = dens_cont_bmlvar_sparse1,
      outstrength = outstrength_bmlvar_sparse1,
      strength = strength_bmlvar_sparse1,
      reg_bmlvar = reg_bmlvar_sparse1,
      ests_bmlvar = ests_bmlvar_sparse1,
      summary_bmlvar = summary_bmlvar_sparse1,
      rhat_bmlvar_mean = rhat_bmlvar_mean_sparse1,
      rhat_bmlvar_11 = rhat_bmlvar_11_sparse1,
      divtrans_bmlvar = divtrans_bmlvar_sparse1
    ),
    bmlvar_sparse2 = list(
      fit_bmlvar_sparse2 = list(
        beta = ests_bmlvar_sparse2$beta_est$median,
        pcor = ests_bmlvar_sparse2$pcor_est$median
      ),
      dens_temp = dens_temp_bmlvar_sparse2,
      dens_cont = dens_cont_bmlvar_sparse2,
      outstrength = outstrength_bmlvar_sparse2,
      strength = strength_bmlvar_sparse2,
      reg_bmlvar = reg_bmlvar_sparse2,
      ests_bmlvar = ests_bmlvar_sparse2,
      summary_bmlvar = summary_bmlvar_sparse2,
      rhat_bmlvar_mean = rhat_bmlvar_mean_sparse2,
      rhat_bmlvar_11 = rhat_bmlvar_11_sparse2,
      divtrans_bmlvar = divtrans_bmlvar_sparse2
    ),
    bmlvar_sparse3 = list(
      fit_bmlvar_sparse3 = list(
        beta = ests_bmlvar_sparse3$beta_est$median,
        pcor = ests_bmlvar_sparse3$pcor_est$median
      ),
      dens_temp = dens_temp_bmlvar_sparse3,
      dens_cont = dens_cont_bmlvar_sparse3,
      outstrength = outstrength_bmlvar_sparse3,
      strength = strength_bmlvar_sparse3,
      reg_bmlvar = reg_bmlvar_sparse3,
      ests_bmlvar = ests_bmlvar_sparse3,
      summary_bmlvar = summary_bmlvar_sparse3,
      rhat_bmlvar_mean = rhat_bmlvar_mean_sparse3,
      rhat_bmlvar_11 = rhat_bmlvar_11_sparse3,
      divtrans_bmlvar = divtrans_bmlvar_sparse3
    ),
...
sim_summarise <- function(condition, results, fixed_objects = NULL){
  ...
  #   # Summary for bmlvar
  # method <- c("bmlvar_sparse1", "bmlvar_sparse2", "bmlvar_sparse3")
  ...
  #--- Return
  ret$bmlvar_sparse1_diagnostics <- list()
  ret$bmlvar_diagnostics$rhat_bmlvar_mean <- mean(sapply(results, function(x){
     x$bmlvar_sparse1$rhat_bmlvar_mean}))
  ret$bmlvar_sparse1_diagnostics$rhat_bmlvar_mean <- mean(sapply(results, function(x){
     x$bmlvar_sparse1$rhat_bmlvar_11}))
  ret$bmlvar_sparse1_diagnostics$divtrans_bmlvar <- lapply(results, function(x){
    x$bmlvar_sparse1$divtrans_bmlvar})

     ret$bmlvar_sparse2_diagnostics <- list()
  ret$bmlvar_diagnostics$rhat_bmlvar_mean <- mean(sapply(results, function(x){
     x$bmlvar_sparse2$rhat_bmlvar_mean}))
  ret$bmlvar_sparse2_diagnostics$rhat_bmlvar_mean <- mean(sapply(results, function(x){
     x$bmlvar_sparse2$rhat_bmlvar_11}))
  ret$bmlvar_sparse2_diagnostics$divtrans_bmlvar <- lapply(results, function(x){
    x$bmlvar_sparse2$divtrans_bmlvar})

      ret$bmlvar_sparse3_diagnostics <- list()
  ret$bmlvar_diagnostics$rhat_bmlvar_mean <- mean(sapply(results, function(x){
     x$bmlvar_sparse3$rhat_bmlvar_mean}))
  ret$bmlvar_sparse3_diagnostics$rhat_bmlvar_mean <- mean(sapply(results, function(x){
     x$bmlvar_sparse3$rhat_bmlvar_11}))
  ret$bmlvar_sparse3_diagnostics$divtrans_bmlvar <- lapply(results, function(x){
    x$bmlvar_sparse3$divtrans_bmlvar}) 
