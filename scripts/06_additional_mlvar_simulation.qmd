...
## Idea 1.1.: Low innovation variance
...
Investigate the results. The average rank correlation for temporal coefficients is effectively zero. We only have five repetitions here, but the pattern is clear: 
```{r}
sim_res_mlvar <- readRDS(here("output", "low_kappa_sd_mlvar_results", "low_kappa_sd_mlvar.rds"))

sim_res_mlvar |> 
  select(rankcor_temp_mlvar_mean, rankcor_temp_mlvar_mcse) |> 
  knitr::kable()
```
...
```{r data-analysis-sparse, eval=FALSE}
sim_analyse <- function(condition, dat, fixed_objects = NULL){
...
We now investigate the results with a focus on outcome prediction: 
```{r}
sim_res_sparse <- readRDS(here::here("output", "additional_sparser_dgp_results", "additional_sparser_dgp.rds"))

sim_res_sparse |> 
  select(n_id, n_tp, power_reg_outstrength_mlvar_mean.power_reg4_outstrength_mlvar_mean) |> 
  dplyr::rename("Mean Power for 0.4 correlation" = power_reg_outstrength_mlvar_mean.power_reg4_outstrength_mlvar_mean) |> 
  knitr::kable()
```
The power is still bad. 
...
We can additionally check network density (although this was not explicitly manipulated here, so there will be less true score variance). The results don't seem to be much different: 

```{r}
density_list_mlvar <- list()

for (i in seq_along(sim_res_4$results)) {
  
    mlvar_density = sapply(sim_res_4$results[[i]]$mlvar$outstrength, sum)
    true_density = sapply(sim_res_4$results[[i]]$true_cent$outstrength, sum)
  ...
  density_list_mlvar[[i]] <- df_temp
}

df_density_mlvar <- do.call(rbind, density_list_mlvar)

# add an identifier column to track which element each row came from
df_density_mlvar$sim_rep <- rep(seq_along(sim_res_4$results), sapply(density_list_mlvar, nrow))


df_density_mlvar |> 
  ggplot(aes(x = true_density, y = mlvar_density)) + 
  geom_point() + 
  geom_smooth() + 
  theme_centrality()+
  labs(x = "True Density",
...
Alternatively, we can calculate the correlation between true and estimated density for each iteration of the simulation: 
```{r}
df_density_mlvar |> 
  group_by(sim_rep) |> 
  summarize(true_est_cor = cor(true_density, mlvar_density)) |> 
  ggplot(aes(x = true_est_cor))+
  geom_histogram()+ 
  theme_centrality()+
  labs(x = "Correlation of true and estimated densities")
```
...
## Compare estimate correlations

We can additionally compare the correlation between different edge estimates between BmlVAR and mLVAR to understand if the univariate estimation by mlVAR leads to striking differences here. We observe no large differences.

```{r}
# extract beta estimates
mlvar_betas <- lapply(sim_res_4$results, function(x){
  lapply(x$mlvar$fit_mlvar$beta, function(y){
    as.vector(y)
  })
}) |> 
    unlist(recursive = FALSE)

df_mlvar_betas <- as.data.frame(do.call(rbind, mlvar_betas)) |> 
  mutate(method = "mlvar") |> 
  mutate(sim_iter = rep(1:200, each = 200))

bmlvar_betas <- lapply(sim_res_4$results, function(x){
  lapply(x$bmlvar$fit_bmlvar$beta, function(y){
    as.vector(y)
  })
}) |> 
    unlist(recursive = FALSE)

df_bmlvar_betas <- as.data.frame(do.call(rbind, bmlvar_betas)) |> 
  mutate(method = "bmlvar") |> 
  mutate(sim_iter = rep(1:200, each = 200))

df_betas <- bind_rows(df_mlvar_betas, df_bmlvar_betas)


df_split <- df_betas |> 
  split(list(.$sim_iter, .$method), drop = TRUE)

cor_matrices <- map(df_split, ~ cor(select(.x, starts_with("V")), use = "pairwise.complete.obs"))


cor_df <- tibble(
  sim_iter = as.integer(str_extract(names(cor_matrices), "^\\d+")),  # Extract iteration number
  method = str_extract(names(cor_matrices), "(mlvar|bmlvar)"),       # Extract method name
  cor_matrix = cor_matrices                                          # Store correlation matrix
)

# compute average absolute non-diagonal correlation
mean_abs_nondiag_cor <- function(x){
  x[x = 1] <- 0
  x <- x[!is.na(x)]
  mean(abs(unlist(x)))
}
...
cor_df |> 
  mutate(mean_cor = as.numeric(map(cor_matrix, mean_abs_nondiag_cor))) |> 
  dplyr::group_by(method) |> 
  dplyr::summarize(mean_abs_cor = mean(mean_cor), 
            sd_abs_cor = sd(mean_cor)) |> 
  knitr::kable()
...
We use a graph obtained by estimating mlVAR models on data by Fried et al. (2022). More information on the fitting process can be found in the file `04_dgps.qmd`. 
...
# Idea 5: Different mlVAR Estimator

We can also try a different self-made estimator for a multilevel VAR model in a frequentist framework. We build it by estimating univariate regressions with nlme. 
...
# simulate data from a different simulation function
mlvar_sim <- mlVAR::mlVARsim(nPerson = 60, 
                nNode = 5, 
                nTime = 60, 
                lag = 1)
df_data <- mlvar_sim$Data |> 
  mutate(ID = as.factor(ID)) |> 
  group_by(ID) |> 
  mutate(tp = row_number()) |>
  ungroup()
