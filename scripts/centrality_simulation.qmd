...
sim_generate <- function(condition, fixed_objects = NULL){
  ...
  kappa_sd_low <- 0.1 * mean_diag_kappa
  kappa_sd_high <- 0.3 * mean_diag_kappa
...
sim_analyse <- function(condition, dat, fixed_objects = NULL){
  ...
  ret_results <- list(
    ...
    bmlvar = list(
      ...
      # reg_tempdens = reg_bmlvar_temp_dens,
      # reg_cont_dens = reg_bmlvar_cont_dens,
...
- GVAR fit: already transposed into rows as lagged, cols as contemp
- mlVAR fit: rows as lagged, cols as contemp
- gimme fit: already transposed into rows as lagged, cols as contemp
...
sim_summarise <- function(condition, results, fixed_objects = NULL){
      ...
      for (measure in measures) {
        for (summary in summaries) {
          ...
          if(func_name == "rmse"){
            ...
            ret[[paste0(result_name_rmse, "_mcse")]] <- sqrt ( 
              apply(calc_list, 1, var, na.rm = TRUE) / (4*n_rep*mse_mean_tmp))
          }
          else if(func_name == "mse"){
            ret[[paste0(result_name_rmse, "_mcse")]] <- sqrt(
          ...
          ret[[paste0(result_name_power, "_mcse")]] <- sqrt(
  ...
  mean_pwr_oneside <- rowMeans(calc_pwr_oneside, na.rm = TRUE)
  mean_pwr_twoside <- rowMeans(calc_pwr_twoside, na.rm = TRUE)
  
  # TODO
  
  
  #--- Return
  # TODO the regression stuff with 3 measures might take some renaming
...
n_var <- 3
n_id <- 25
sim_pars$graph_sparse$beta <- sim_pars$graph_sparse$beta[1:3,1:3]
sim_pars$graph_sparse$pcor <- sim_pars$graph_sparse$pcor[1:3,1:3]
sim_pars$graph_sparse$kappa <- sim_pars$graph_sparse$kappa[1:3,1:3]
sim_pars$graph_nonsparse$beta <- sim_pars$graph_nonsparse$beta[1:3,1:3]
sim_pars$graph_nonsparse$pcor <- sim_pars$graph_nonsparse$pcor[1:3,1:3]
sim_pars$graph_nonsparse$kappa <- sim_pars$graph_nonsparse$kappa[1:3,1:3]
...
SimClean()
saveRDS(sim_results, file = "output/single_row_results.RDS")
...
```{r mock-results}
sim_res <- readRDS("output/single_row_results.RDS")
...
```{r}
sr_edit <- sim_res %>% 
  mutate(dgp = factor(dgp, levels = c("nonsparse", "sparse")),
...
Plot point estimate recovery (RMSE), not directly relevant for the manuscript
...
ggsave("plot_mostcentral_mock.svg", plot_mostcentral, height = 6, width = 9,
