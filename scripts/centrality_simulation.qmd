...
sim_analyse <- function(condition, dat, fixed_objects = NULL){
  ...
  outstrength_gvar <- sapply(cent_gvar$outstrength, function(x) unname(x))
  instrength_gvar <- sapply(cent_gvar$instrength, function(x) unname(x))
  ...
  outstrength_gimme <- sapply(cent_gimme$outstrength, function(x) unname(x))
  instrength_gimme <- sapply(cent_gimme$instrength, function(x) unname(x))
  ...
  outstrength_mlvar <- sapply(cent_mlvar$outstrength, function(x) unname(x))
  instrength_mlvar <- sapply(cent_mlvar$instrength, function(x) unname(x))
  ...
  outstrength_blmvar <- sapply(cent_blmvar$outstrength, function(x) unname(x))
  instrength_blmvar <- sapply(cent_blmvar$instrength, function(x) unname(x))
...
sim_summarise <- function(condition, results, fixed_objects = NULL){
  ...
  rmsemcse_beta_gimme <- sqrt(stats::var(rmse_beta_gimme_mean_tmp) / (4*n_rep*mse_beta_gimme_mean))
  rmsemcse_pcor_gimme <- sqrt(stats::var(rmse_pcor_gimme_mean_tmp) / (4*n_rep*mse_pcor_gimme_mean))
  ...
  rmsemcse_beta_mlvar <- sqrt(stats::var(rmse_beta_mlvar_mean_tmp) / (4*n_rep*mse_beta_mlvar_mean))
  rmsemcse_pcor_mlvar <- sqrt(stats::var(rmse_pcor_mlvar_mean_tmp) / (4*n_rep*mse_pcor_mlvar_mean))
  ...
  rmsemcse_beta_bmlvar <- sqrt(stats::var(rmse_beta_bmlvar_mean_tmp) / (4*n_rep*mse_beta_bmlvar_mean))
  rmsemcse_pcor_bmlvar <- sqrt(stats::var(rmse_pcor_bmlvar_mean_tmp) / (4*n_rep*mse_pcor_bmlvar_mean))
  ...
  mostcent_beta_gvar <- lapply(seq_along(results), function(i){
    if(is.null(results[[i]]$gvar$fit_gvar)){
      ...
      most_cent_ident(results[[i]]$gvar$fit_gvar$outstrength, 
                      results[[i]]$true_cent$outstrength)
    }
  ...
  #- LASSO GVAR
  rmse_reg_tempdens_gvar <- regression_rmse(results, "gvar", "tempdens")
  rmse_reg_contdens_gvar <- regression_rmse(results, "gvar", "contdens")
  rmse_reg_outstrength_gvar <- regression_rmse(results, "gvar", "outstrength")
  
    
  #- GIMME
  rmse_reg_tempdens_gimme <- regression_rmse(results, "gimme", "tempdens")
  rmse_reg_contdens_gimme <- regression_rmse(results, "gimme", "contdens")
  rmse_reg_outstrength_gimme <- regression_rmse(results, "gimme", "outstrength")
  
  #- mlVAR
  rmse_reg_tempdens_mlvar <- regression_rmse(results, "mlvar", "tempdens")
  rmse_reg_contdens_mlvar <- regression_rmse(results, "mlvar", "contdens")
  rmse_reg_outstrength_mlvar <- regression_rmse(results, "mlvar", "outstrength")
    
  #- BmlVar
  rmse_reg_tempdens_bmlvar <- regression_rmse(results, "bmlvar", "tempdens")
  rmse_reg_contdens_bmlvar <- regression_rmse(results, "bmlvar", "contdens")
  rmse_reg_outstrength_bmlvar <- regression_rmse(results, "bmlvar", "outstrength")
