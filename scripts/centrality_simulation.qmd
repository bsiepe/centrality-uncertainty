...
sim_generate <- function(condition, fixed_objects = NULL){
  ...
  kappa_sd_high <- 0.2 * mean_diag_kappa
...
sim_analyse <- function(condition, dat, fixed_objects = NULL){
  ...
  fit_gvar <- lapply(dat$data, function(x){
    tryCatch({graphicalVAR::graphicalVAR(x, 
                               nLambda = 50,
                               verbose = FALSE,
                               gamma = 0, 
  ...
  # TODO add check for graphicalVAR
  ...
  # TODO add check for GIMME as well
  ...
  rhat_bmlvar <- apply(posterior::as_draws_array(fit_bmlvar), 3, rstan::Rhat)
...
sim_summarise <- function(condition, results, fixed_objects = NULL){
  ...
  ret$bmlvar_diagnostics$rhat_bmlvar <- lapply(results, function(x){
    x$bmlvar$rhat_bmlvar})
...
SimClean()
saveRDS(sim_results, file = here("output/single_row_results.RDS"))
...
```{r mock-results}
sim_res <- readRDS(here("output/single_row_results.RDS"))
