...
sim_summarise <- function(condition, results, fixed_objects = NULL){
  ...
  for (method in methods) {
    for (measure in measures) {
      result_name <- paste("rankcor", method, measure, sep = "_")
...
```{r viz-prep}
meth_colors <- set_names(ggokabeito::palette_okabe_ito()[c(5, 1, 2, 3)],
...
theme_centrality <- function(){
  ...
  # add horizontal line between factors (i.e., methods)
  ...
  ggplot2::theme_minimal(base_family = "news") +
    ggplot2::theme(
      ...
      axis.text.x = ggplot2::element_text(face = "plain", 
                                          size = ggplot2::rel(1.2)),
...
Plot point estimate recovery (RMSE)
