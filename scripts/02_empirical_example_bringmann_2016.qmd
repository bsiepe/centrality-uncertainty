...
data_list <- split(df_data_scaled, ~ subj_id)
...
fit_gvar <- lapply(data_list_scaled, function(x){
...
reg_df_gvar <- data.frame(neuroticism = neuroticism_vec_scaled,
...
pre_mlvar <- Sys.time()
fit_mlvar <- tryCatch({suppressMessages(mlVAR::mlVAR(df_data_scaled,
                            vars = vars,
                            idvar = "subj_id",
                            estimator = "lmer",
...
# Combine into dataframe with neuroticism
reg_df_mlvar <- data.frame(neuroticism = neuroticism_vec_scaled,
...
data_list <- lapply(data_list_scaled, function(x){
    x |> 
      select(-c(subj_id))
  }) 
...
# Combine into dataframe with neuroticism
reg_df_gimme <- data.frame(neuroticism = neuroticism_vec_scaled,
