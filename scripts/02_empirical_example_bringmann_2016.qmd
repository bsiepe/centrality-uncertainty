...
TODO check convergence problems here
...
TODO need to scale data before for every method!
...
# save results
saveRDS(fit_gvar, here("empirical_example", "fit_gvar.rds"))

```

```{r, include = FALSE}
fit_gvar <- readRDS(here("empirical_example", "fit_gvar.rds"))
```
...
reg_df_gvar <- data.frame(neuroticism = neuroticism_vec,
...
saveRDS(fit_mlvar, here("empirical_example", "fit_mlvar.rds"))
```

```{r, include = FALSE}
fit_mlvar <- readRDS(here("empirical_example", "fit_mlvar.rds"))
```
...
dens_temp_mlvar <- cent_mlvar$dens_temp
outstrength_mlvar_first <- sapply(cent_mlvar$outstrength, function(x) unname(x[1]))

# Combine into dataframe with neuroticism
reg_df_mlvar <- data.frame(neuroticism = neuroticism_vec,
...
```{r fit-gimme}
#| eval: !expr params$rerun
fit_gimme <- tryCatch({gimme::gimme(data_list,
                            ar = TRUE,
                            subgroup = TRUE,
                            plot = FALSE,
                            hybrid = FALSE,
...
saveRDS(fit_gimme, here("empirical_example", "fit_gimme.rds"))
```

```{r, include = FALSE}
fit_gimme <- readRDS(here("empirical_example", "fit_gimme.rds"))
```
...
# Combine into dataframe with neuroticism
reg_df_gimme <- data.frame(neuroticism = neuroticism_vec,
