...
most_central_example_plot <- outstrength_long_labeled |> 
  left_join(proportions, by = c("id" = "id", "variable" = "variable")) |> 
  dplyr::mutate(id = paste0("Participant ", id)) |> 
  dplyr::mutate(variable = factor(variable)) |> 
  ggplot(aes(x = outstrength, y = variable, alpha = cent_alpha)) + 
  ggdist::stat_halfeye(.width = c(.66, .95), 
                       fill = cond_colors[4],
                       aes(slab_alpha = cent_alpha),
                       point_fill = "grey70",
                       point_color = "grey70",
                       color = "grey70",
                       point_alpha = 1,
  ...
  labs(y = "", x = "Outstrength",
       title = "We can quantify the uncertainty of node centrality") +
  guides(alpha = "none")

most_central_example_plot

ggsave(here::here("figures", "empirical_example", "most_central_example_plot_presentation.pdf"), width = 9 * 0.9, height = 6 * 0.9)
```
