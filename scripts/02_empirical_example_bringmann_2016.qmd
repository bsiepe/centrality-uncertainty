...
subj_to_exclude <- data_bringmann |> 
  ...
  dplyr::summarise(across(everything(),
                   ~var(.x, na.rm = TRUE))) |> 
  dplyr::filter(if_any(everything(), ~.x < 1)) |> 
  dplyr::pull(subj_id)

data_bringmann <- data_bringmann |> 
  dplyr::filter(!subj_id %in% subj_to_exclude)

neuroticism_vec_clean <- data_bringmann |> 
  dplyr::group_by(subj_id) |> 
  dplyr::distinct(neuroticism) |>
  dplyr::pull(neuroticism)
...
```{r}
n_non_miss <- summary_table |> 
  dplyr::select(dplyr::ends_with("_obs")) |>
  unlist() |>
  mean()
  
```

The average number of non-missing observations is `r round(n_non_miss, 2)`.
...
data_bringmann_imputed <- data_bringmann |> 
  dplyr::select(-neuroticism) |> 
  dplyr::group_by(subj_id) |> 
  dplyr::mutate(across(everything(),
                ~imputeTS::na_kalman(.x, optim.control = new_param),
                .names = "{.col}_imp")) |> 
  dplyr::ungroup() |> 
  dplyr::left_join(neuroticism_vec, by = "subj_id")
...
pdf(here::here("figures", "empirical_example_imputations.pdf"))  # Customize width and height as needed
for (i in 1:length(convergence_ids)) {
  for (col in 1:length(columns)) {
    
    dat1 <- data_bringmann_imputed |> 
      dplyr::filter(subj_id %in% convergence_ids[[i]])
...
data_bringmann_imputed <- data_bringmann_imputed |> 
  dplyr::select(subj_id, contains("_imp"), neuroticism) |>
  dplyr::rename_with(~str_remove(.x, "_imp"), contains("_imp"))

```

What is the final number of time points per person?

```{r}
n_final <- data_bringmann_imputed |> 
  dplyr::count(subj_id) |> 
  summarize(avg_n = mean(n))
```

After imputation, it is `r round(n_final, 2)`
...
df_data_scaled <- df_data |> 
  dplyr::group_by(subj_id) |> 
  dplyr::mutate(across(everything(), ~scale(.x))) |> 
  dplyr::ungroup()
...
df_data_centered <- df_data |> 
  dplyr::group_by(subj_id) |> 
  dplyr::mutate(across(everything(), ~scale(.x, scale = FALSE))) |> 
  dplyr::ungroup()
...
# save results
saveRDS(fit_gvar_scaled, here::here("output", "empirical_example", "fit_gvar_scaled.rds"))
...
# save results
saveRDS(fit_gvar_centered, here::here("output", "empirical_example", "fit_gvar_centered.rds"))


```

```{r, include = FALSE}
fit_gvar_scaled <- readRDS(here::here("output", "empirical_example", "fit_gvar_scaled.rds"))
fit_gvar_centered <- readRDS(here::here("output", "empirical_example", "fit_gvar_centered.rds"))
```
...
```{r, include = FALSE}
fit_mlvar_scaled <- readRDS(here::here("output", "empirical_example", "fit_mlvar_scaled.rds"))
fit_mlvar_centered <- readRDS(here::here("output", "empirical_example", "fit_mlvar_centered.rds"))
```
...
```{r centrality-mlvar}
# scaled
cent_mlvar_scaled <- centrality_mlvar(fit_mlvar_scaled)
...
# centered
cent_mlvar_centered <- centrality_mlvar(fit_mlvar_centered)
...
reg_df_mlvar_centered <- data.frame(neuroticism = neuroticism_vec_scaled,
...
```{r reg-mlvar-dens}
reg_mlvar_centered_dens <- lm(neuroticism ~ dens_temp, data = reg_df_mlvar_centered)
...
saveRDS(fit_gimme_scaled, here::here("output", "empirical_example", "fit_gimme_scaled.rds"))
...
fit_gimme_centered <- gimme::gimme(data_list_centered_gimme,
                            ...
                            subcutoff = .75,
                            VAR = TRUE)
saveRDS(fit_gimme_centered, here::here("output", "empirical_example", "fit_gimme_centered.rds"))

```

```{r, include = FALSE}
fit_gimme_scaled <- readRDS(here::here("output", "empirical_example", "fit_gimme_scaled.rds"))
fit_gimme_centered <- readRDS(here::here("output", "empirical_example", "fit_gimme_centered.rds"))
```
...
Specifically, we need the following input: 

    - The number of variables (K)
    - The number of individuals (I)
    - The number of regression outcomes (P)
    - The total number of observations (N_total)
    - The number of time points (n_t)
    - The number of partial correlations (n_pc)
    - The indicators for the upper triangle of the partial correlations (idx_rho)
    - The scaled data (Y)
    - The regression predictors (reg_covariate)
    - The setting for the sparsity prior (sparsity): 


For the sparsity prior on the temporal network, we implemented three different options: 
...
# We find the indicators for the upper triangle for the partial correlations
idx_rho <- upper.tri(matrix(1, n_var, n_var, byrow = F)) |>  
      c() |>  
      which()
  
# As we use two predictors, we need to duplicate the outcome vector
reg_data <- cbind(
    neuroticism_vec_scaled,
    neuroticism_vec_scaled
  )

# We provide the scaled data in matrix form
Y_scaled <- df_data_scaled |> 
    dplyr::select(-c("subj_id")) |>  
    as.matrix()

# We provide the unscaled data in matrix form
Y <- df_data |> 
    dplyr::select(-c("subj_id")) |>  
    as.matrix()

# We obtain the number of time points 
...
stan_data_scaled <- list(
    ...
    # indicators for the upper triangle of the partial correlations
    idx_rho = array(idx_rho, dim = length(idx_rho)),
    # scaled data
    Y = Y_scaled,
    # regression predictors
    reg_covariate = reg_data,
    # setting for the sparsity prior
    sparsity = 2,
    # should mean-centering be performed or not?
    mean_center = 0
  )
...
bmlvar_fit_time
saveRDS(fit_bmlvar_scaled, here::here("output", "empirical_example", "fit_bmlvar_empirical_scaled.rds"))
...
saveRDS(fit_bmlvar_centered, here::here("output", "empirical_example", "fit_bmlvar_empirical_centered.rds"))
...
```{r centrality-bmlvar}
#| eval: !expr params$rerun
fit_bmlvar_scaled <- readRDS(here::here("output", "empirical_example", "fit_bmlvar_empirical_scaled.rds"))
fit_bmlvar_centered <- readRDS(here::here("output", "empirical_example", "fit_bmlvar_empirical_centered.rds"))
...
saveRDS(df_regression_scaled, here::here("output/empirical_example/regression_empirical_example_scaled.RDS"))
...
saveRDS(df_regression_centered, here::here("output/empirical_example/regression_empirical_example_centered.RDS"))

rm(ests_bmlvar_centered)
rm(fit_bmlvar_centered)
```



# Contrast results 
In this section, we collect and contrast the results of different methods. For now, we analyze results of scaled data.
```{r load-results, include = FALSE}
df_regression_scaled <- readRDS(here::here("output", "empirical_example", "regression_empirical_example_scaled.RDS"))
```
...
df_outstrength <- data.frame(
  ...
  est_outstrength = c(
    df_regression_scaled$gvar$est_outstrength,
    df_regression_scaled$gimme$est_outstrength,
    df_regression_scaled$mlvar$est_outstrength,
    df_regression_scaled$bmlvar$est_outstrength
  ),
  id = rep(seq(1:n_id), 4)
)
...
outliers <- df_outstrength |> 
  dplyr::filter(est_outstrength > y_limit) |> 
  dplyr::mutate(method = factor(method,
                         levels = c("GVAR",  "GIMME", "mlVAR", "BmlVAR"))) |> 
  dplyr::group_by(method) |> 
  dplyr::summarize(outlier_values = paste0(round(est_outstrength, 3), collapse = ", "),
            max_outlier = max(est_outstrength)) |> 
  dplyr::mutate(outlier_values = if_else(method == "GVAR", 
...
outstrength_dist <- df_outstrength |> 
  ...
  geom_segment(data = outliers,
               aes(x = as.numeric(method), xend = as.numeric(method),
                   y =  y_limit * 0.85, yend =  y_limit),
               arrow = arrow(length = unit(0.2, "cm")),
               color = "grey30")+
  geom_text(data = outliers,
            aes(x = as.numeric(method) + .09, 
                y = y_limit * 0.925, label = outlier_values),
            color = "grey30", size = 4, inherit.aes = FALSE, hjust = 0)

outstrength_dist
ggsave(here::here("figures/empirical_example/outstrength_distribution.pdf"), width = 9 * 0.8, height = 6 * 0.8)
```
...
df_density <- data.frame(
  method = rep(c("GVAR", "GIMME", "mlVAR", "BmlVAR"), each = n_id),
  est_dens = c(
    df_regression_scaled$gvar$est_dens,
    df_regression_scaled$mlvar$est_dens,
    df_regression_scaled$gimme$est_dens,
    df_regression_scaled$bmlvar$est_dens
  )
)
...
df_estimates_scaled <- data.frame(
  Method = rep(methods, times = 2),
  Outcome = rep(outcomes, each = 4),
  Point_Estimate = c(
    get_standardized_estimate(reg_gvar_scaled_dens, "estimate"),
    get_standardized_estimate(reg_gimme_scaled_dens, "estimate"),
    get_standardized_estimate(reg_mlvar_scaled_dens, "estimate"),
    bmlvar_scaled_reg_standardized[2,]$summary.mean,
    get_standardized_estimate(reg_gvar_scaled_outstrength, "estimate"),
    get_standardized_estimate(reg_gimme_scaled_outstrength, "estimate"),
    get_standardized_estimate(reg_mlvar_scaled_outstrength, "estimate"),
    bmlvar_scaled_reg_standardized[1,]$summary.mean
  ),
  Lower_Limit = c(
    get_standardized_estimate(reg_gvar_scaled_dens, "lower_ci"),
    get_standardized_estimate(reg_gimme_scaled_dens, "lower_ci"),
    get_standardized_estimate(reg_mlvar_scaled_dens, "lower_ci"),
    bmlvar_scaled_reg_standardized[2,]$summary.2.5.,
    get_standardized_estimate(reg_gvar_scaled_outstrength, "lower_ci"),
    get_standardized_estimate(reg_gimme_scaled_outstrength, "lower_ci"),
    get_standardized_estimate(reg_mlvar_scaled_outstrength, "lower_ci"),
    bmlvar_scaled_reg_standardized[1,]$summary.2.5.
  ),
  Upper_Limit = c(
    get_standardized_estimate(reg_gvar_scaled_dens, "upper_ci"),
    get_standardized_estimate(reg_gimme_scaled_dens, "upper_ci"),
    get_standardized_estimate(reg_mlvar_scaled_dens, "upper_ci"),
    bmlvar_scaled_reg_standardized[2,]$summary.97.5.,
    get_standardized_estimate(reg_gvar_scaled_outstrength, "upper_ci"),
    get_standardized_estimate(reg_gimme_scaled_outstrength, "upper_ci"),
    get_standardized_estimate(reg_mlvar_scaled_outstrength, "upper_ci"),
    bmlvar_scaled_reg_standardized[1,]$summary.97.5.
  )
)
df_estimates_scaled <- df_estimates_scaled |> 
...
min_val_est <- min(df_estimates_scaled$Density_Point_Estimate, df_estimates_scaled$Outstrength_Point_Estimate)
max_val_est <- max(df_estimates_scaled$Density_Point_Estimate, df_estimates_scaled$Outstrength_Point_Estimate)

# set minimum and maximum values symmetrically
min_val <- -max(abs(min_val_est), abs(max_val_est))
max_val <- max(abs(min_val_est), abs(max_val_est))

gt_table <- df_estimates_scaled |> 
...
# save to LaTeX
gt_table |> 
  gtsave(here::here("output", "empirical_example", "summary_table.tex"))
...
fit_bmlvar <- readRDS(here::here("output", "empirical_example", "fit_bmlvar_empirical_scaled.rds"))
...
total_iterations <- outstrength_long |>
  dplyr::group_by(id) |>
  dplyr::summarise(total = n_distinct(iteration), .groups = "drop") 
...
complete_counts <- all_combinations |>
  ...
  dplyr::mutate(count = replace_na(count, 0)) # Replace NA counts with 0

proportions <- complete_counts |>
  left_join(total_iterations, by = "id") |>
  dplyr::mutate(proportion = count / total) |>
  dplyr::select(id, variable, proportion)
...
#| eval: !expr params$rerun
outstrength_bmlvar <- ests_bmlvar_scaled$outstrength_est
...
extreme_props <- outstrength_df |> 
  ...
  dplyr::mutate(most_cent = as.numeric(most_cent)) |> 
  dplyr::mutate(most_cent = case_match(most_cent, 
    1 ~ var_names[1],
    2 ~ var_names[2],
    3 ~ var_names[3],
    4 ~ var_names[4],
...
proportion_labels <- proportions |> 
  ...
  dplyr::mutate(id = paste0("Participant ", id)) |> 
  dplyr::mutate(proportion = sub("^(-?)0.", "\\1.",sprintf("%.2f", proportion))) |> 
  dplyr::mutate(variable = factor(variable, levels = 1:6, labels = var_names))
...
proportion_labels <- proportion_labels |>
  ...
  left_join(extreme_props |>
            ...
            by = "id") |>
  # change alpha based on most cent
  dplyr::mutate(cent_alpha = if_else(variable == most_cent, 
                         1, 0.9))
...
proportion_title <- proportion_labels |> 
  dplyr::group_by(id) |> 
  dplyr::mutate(proportion_title = "p(c[p] == c[max])") |> 
  distinct(id, proportion_title) |> 
  dplyr::mutate(x = 0.19, y = 6.5)
...
```{r}
#| eval: !expr params$rerun
# add information about most central node
extreme_props <- extreme_props |> 
  as.data.frame() |> 
  dplyr::mutate(most_cent = as.character(most_cent)) |> 
  dplyr::mutate(id = as.character(id))

outstrength_long_labeled <- outstrength_long |> 
  dplyr::mutate(variable = as.character(variable)) |> 
  ...
  dplyr::mutate(id = as.character(id)) |> 
  filter(id %in% extreme_ids) |> 
  left_join(extreme_props |> select(id, most_cent), by = "id") |> 
  dplyr::mutate(cent_alpha = if_else(variable == most_cent, 1, 0.60)) |> 
  dplyr::mutate(interval_color = if_else(variable == most_cent, "grey70", "grey10"))
...
most_central_data <- outstrength_long_labeled |> 
  left_join(proportions, by = c("id" = "id", "variable" = "variable")) |> 
  dplyr::mutate(id = paste0("Participant ", id)) |> 
  dplyr::mutate(variable = factor(variable)) |> 
  filter(variable == most_cent)

other_data <- outstrength_long_labeled |> 
  left_join(proportions, by = c("id" = "id", "variable" = "variable")) |> 
  dplyr::mutate(id = paste0("Participant ", id)) |> 
  dplyr::mutate(variable = factor(variable)) |> 
  filter(variable != most_cent)

most_central_example_plot <- ggplot() + 
  # non-most central variables
  ggdist::stat_halfeye(data = other_data,
                       aes(x = outstrength, y = variable, slab_alpha = cent_alpha),
                       .width = c(.66, .95), 
                       fill = cond_colors[4],
                       color = "grey10",
                       point_fill = "grey10",
  ...
  ggdist::stat_halfeye(data = most_central_data,
                       aes(x = outstrength, y = variable, slab_alpha = cent_alpha),
                       .width = c(.66, .95), 
                       fill = cond_colors[4],
                       color = "grey70",
  ...
  geom_text(data = proportion_labels,
            aes(x = 0.19, y = variable, label = proportion),
  ...
  facet_wrap(~id, axes = "all") +
  theme_centrality() +
  theme(axis.text.y = element_text(size = 14),
        axis.text.x = element_text(size = 14),
        strip.text.x.top = element_text(size = 17),
...
ggsave(here::here("figures", "empirical_example", "most_central_example_plot.pdf"), width = 9 * 0.9, height = 6 * 0.9)
```

## Prior Predictive

To better understand our model, we can simulate from our priors. We can use that, for example, to understand the implied prior on the outstrength centrality. We do not place a prior directly on the centrality, but rather on the individual coefficients going into its sum. 

```{r}
#| label: bmlvar-priorpred
n_sim <- 100000

# Simulate from Cauchy(0, 0.05)
# rcauchy uses location and scale parameters
cauchy_samples <- matrix(rcauchy(n_sim * 6, location = 0, scale = 0.05), 
                         nrow = n_sim, ncol = 6)

# obtain single coefficient and sum of coefficients
single_coef <- cauchy_samples[, 1]
abs_sum <- rowSums(abs(cauchy_samples))
...
df_long$type <- factor(df_long$type, 
                       levels = c("single", "abs_sum"),
                       labels = c("Single Coefficient", 
                                  "Implied Outstrength Centrality Prior"))
...
ggplot(df_long, aes(x = value, fill = type)) +
  ...
  xlim(-2, 5) +
  labs(title = "Distribution of Single Cauchy(0, 0.05) vs Absolute Sum of 6",
       x = "Value",
       y = "Density",
       fill = "") +
  theme_minimal() +
...
An additional advantage of Bayesian inference is the use of posterior predictive simulations, where we can compare predictive simulations from the posterior with the true observed data patterns. Here, this indicates that we did not accurately capture the observed data pattern. 
```{r bmlvar-postpred}
y_rep_draws <- rstan::extract(fit_bmlvar, "Y_rep")

posterior_draws <- y_rep_draws$Y_rep

# transpose posterior_draws
posterior_draws <- aperm(y_rep_draws$Y_rep, c(2,1,3))

# Sample 200 columns
posterior_samples <- posterior_draws[, sample(dim(posterior_draws)[2], 200),]

# split into list
samples_list <- plyr::alply(posterior_samples, 3)

# convert to dataframe with id
samples_list <- lapply(samples_list, function(x){
  x |> 
    as.data.frame() |> 
    mutate(subj_id = data_bringmann_imputed$subj_id) |> 
    dplyr::group_by(subj_id) |> 
    dplyr::mutate(obs = row_number()) |> 
    dplyr::ungroup() |> 
    pivot_longer(cols = !c(subj_id, obs),
                 values_to = "est",
                 names_to = "iter") |> 
    dplyr::mutate(iter = as.numeric(gsub("V", "", iter)))
})


# plot example participant with second variable (happy)
samples_list[[2]] |> 
  dplyr::filter(subj_id == "31") |> 
  dplyr::group_by(obs) |> 
  dplyr::summarize(est_mean = mean(est)) |> 
  ungroup() |> 
  # attach observed centered data
  bind_cols(df_data_scaled |> filter(subj_id == 31) |> select(happy)) |> 
  dplyr::rename(estimate = est_mean, 
         observed = happy) |> 
  pivot_longer(!obs) |> 
