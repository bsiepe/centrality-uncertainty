...
```{r}
sim_results <- readRDS(here("sim101024.rds"))
...
sr_edit <- sim_res_cut |> 
  mutate(
    # dgp = factor(dgp, levels = c("dense", "sparse")),
    dgp = factor(dgp, levels = "dense"),
    n_id = factor(n_id, levels = c(75, 200)),
    # heterogeneity = factor(heterogeneity, levels = c("low", "high"))
    heterogeneity = factor(heterogeneity, levels = "low")
    ) |>
  ...
  pivot_longer(cols = -c("dgp", "heterogeneity", "n_tp", "n_id"), 
               names_to = "measure", 
               values_to = "value") |> 
  ...
  group_by(dgp, heterogeneity, n_id, pm, outcome, method) |>
...
power_plot <- function(df) {
  ggplot(df, aes(x = method, 
    ...
    geom_point(position = position_dodge(0.7), 
               size = 1.2) +
    ggh4x::facet_nested(dgp~ outcome + n_id,
                        axes = "all",
                        remove_labels = "y") +
    scale_x_discrete(guide = guide_axis(angle = 90)) +
...
plot_rmse_combined <- cowplot::plot_grid(plotlist = plot_rmse_list, 
                                        ...
                                        labels = c("True Effect: 0", 
                                                   "True Effect: 0.2",
                                                   "True Effect: 0.4"))
ggsave("plot_reg_rmse_combined_prelim_1010.pdf", plot_rmse_combined, height = 12, width = 16,
       path = here::here("figures/"))
