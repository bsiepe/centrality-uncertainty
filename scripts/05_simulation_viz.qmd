...
```{r}
if(isTRUE(use_rev)){
  sim_results <- readRDS(here::here("output", "sim_results_rev1_fixed_sigma.rds"))
} else {
...
| Added heterogeneous most central node condition | The preregistration specified only the homogeneous condition, in which the first node was constrained to be the most central node for all individuals. A coding error revealed that our initial implementation had allowed the most central node to vary between individuals. Upon discovering this, and in line with a reviewer comment,  we recognized the scientific value of comparing both assumptions and retained both conditions in the manuscript. The preregistered homogeneous condition is included as originally planned. |
| Added completely new simulation settings in the second revision | A reviewer suggested to add additional simulation conditions, which are included in `09_additional_simulations_viz.qmd`| 
...
sr_edit <- sim_res_cut |> 
  ...
  pivot_longer(cols = -c("dgp", "n_tp", "n_id"), 
               names_to = "measure", 
               values_to = "value") |> 
  dplyr::mutate(measure = str_replace(measure, "power_reg", "powerreg"),
  ...
  dplyr::filter(!measure %in% c("strength_sd", "outstrength_sd", "instrength_sd")) |> 
 ...
 dplyr::mutate(method = case_when(
  ...
  dplyr::mutate(method = factor(method, levels = c("GVAR", "GIMME", "mlVAR", "BmlVAR"))) |>
  dplyr::mutate(outcome = case_when(
    outcome == "beta" ~ "Temporal",
    outcome == "pcor" ~ "Contemporaneous",
    .default = outcome
  )) |>
  dplyr::group_by(dgp, n_tp, n_id, pm, outcome, method) |>
...
# for the first revision data, this is irrelevant
if(isTRUE(use_rev)){
  sim_full_pre_resum <- readRDS(here("output", "sim_results_rev1_fixed_sigma.rds"))
} else {
...
if(isTRUE(use_rev)){
  ...
  rename(
    "Deprecated dplyr function" = "WARNING:  c(\"Warning in NULL : `funs()` was deprecated in dplyr 0.8.0.\", \"Warning in NULL : Please use a list of either functions or lambdas: \\n\\n  # Simple nam",
    "Bulk ESS low" = "WARNING:  Warning in NULL : Bulk Effective Samples Size (ESS) is too low, indicating posterior means and medians may be unreliable.\nRunning the chains for more i",
    ...
    "Exceeded maximum treedepth" = "WARNING:  Warning in NULL : There were 491 transitions after warmup that exceeded the maximum treedepth. Increase max_treedepth above 10. See\nhttps://mc-stan.org",
...
## Proportion of Correctly Identified Nodes with the Highest Centrality
...
ggsave_rev("plot_mostcentral_presentation.pdf", plot_mostcentral_presentation, height = 6 * 1.2, width = 9 * 1.2,
       path = here::here("figures/"), device = "pdf")
...
### Combining homogeneous and heterogeneous conditions

For the manuscript, we can also combine the homogeneous and heterogeneous conditions into one plot. To do so, we first need to load both results dataframes: 

```{r}
#| label: combine-heterogeneity-most-central

# load heterogeneous simulations
sim_results_het <- readRDS(here::here("output", "sim_results_rev1_heterogeneous.rds"))

# cut irrelevant columns
sim_res_het_cut <- sim_results_het |>
  select(-contains("rhat")) |>
  select(-contains("divtrans"))

sr_edit_het <- sim_res_het_cut |> 
  mutate(
    dgp = factor(n_tp, levels = c(60, 120)),
    n_id = factor(n_id, levels = c(75, 200))
    ) |>
...
plot_mostcentral_het <- sr_edit_het |> 
  # if mcse is missing, set to 0
  mutate(mcse = ifelse(is.na(mcse), 0, mcse)) |>
  mutate(outcome = factor(outcome, levels = c("Temporal", "Contemporaneous"))) |>
  filter(pm == "mostcent") |> 
...
plot_legend_source <- plot_mostcentral + 
  theme(legend.position = "bottom",
...
# extract the legend
legend <- cowplot::get_plot_component(plot_legend_source, 'guide-box-bottom', return_all = TRUE)

# combine the two plots (no labels)
plot_combined <- cowplot::plot_grid(plot_mostcentral + theme(legend.position = "none"),
                                    plot_mostcentral_het + theme(legend.position = "none"),
                                    ncol = 1,
                                    nrow = 2,
...
# add legend below
combined_mostcentral_plot <- cowplot::plot_grid(
  plot_combined,
  legend,
  ncol = 1,
  nrow = 2,
...
ggsave_rev("plot_mostcentral_combined.pdf", combined_mostcentral_plot, height = 11 * 1.2, width = 9 * 1.2,
       path = here::here("figures/"), device = "pdf")
...
## Bias and MSE of Centrality

Based on a reviewer comment, we also added the bias and MSE of the centrality of the first node to this supplement. 
We first need to compute it, as we didn't compute it in our initial simulation. Note that the computation code is commented out so as not to clash with our make file, which forces execution. The results are re-loaded below. 

```{r}
#| label: compute-bias-mse-centrality
#| eval: false
# node1_cent_cond14 <- resummarise_node1_centrality(
#  dir = here::here("output", "rev1_fixed_sigma",
#                   "sim_full_rev1_cond14_fixed_sigma.rds-results_pc04798"),
#  output_file = here::here("output", "node1_centrality_rev1_cond14_fixed_sigma.rds")
# )

# node1_cent_cond23 <- resummarise_node1_centrality(
#  dir = here::here("output", "rev1_fixed_sigma",
#                    "sim_full_rev1_cond23_fixed_sigma.rds-results_pc04669"),
#  output_file = here::here("output", "node1_centrality_rev1_cond23_fixed_sigma.rds")
# )

# node1_cent <- dplyr::bind_rows(node1_cent_cond14, node1_cent_cond23)

# saveRDS(node1_cent,
#        here::here("output", "node1_centrality_rev1_fixed_sigma.rds"))

# node1_cent

```

After computing it once, we can reload it and plot the results: 
```{r}
#| label: plot-bias-mse-centrality
node1_cent <- readRDS(here::here("output", "node1_centrality_rev1_fixed_sigma.rds"))

node1_cent_long <- node1_cent |> 
  dplyr::mutate(
    n_tp = factor(n_tp, levels = c(60, 120)),
    n_id = factor(n_id, levels = c(75, 200))
    ) |>
  # remove "node1_" from all column names
  rename_with(~str_remove(., "node1")) |> 
  dplyr::select(-c("heterogeneity")) |> 
  # pivot longer except conditions cols
  pivot_longer(cols = -c("dgp", "n_tp", "n_id"), 
               names_to = "measure", 
...
Plot MSE: 
```{r}
#| label: plot-mse-centrality
plot_mse <- node1_cent_long |> 
  mutate(mcse = ifelse(is.na(mcse), 0, mcse)) |>
  mutate(outcome = factor(outcome, levels = c("instrength", "outstrength", "strength"))) |>
  filter(pm == "mse") |> 
  ggplot(aes(x = method, 
             y = mean, 
             colour = method
  ...
  labs(title = "",
       x = "Method",
       colour = "Method",
       y = "MSE of Centrality Estimation (1st node)")
...
```{r}
#| label: plot-bias-centrality
plot_bias <- node1_cent_long |> 
  mutate(mcse = ifelse(is.na(mcse), 0, mcse)) |>
  mutate(outcome = factor(outcome, levels = c("instrength", "outstrength", "strength"))) |>
  filter(pm == "bias") |> 
  ggplot(aes(x = method, 
  ...
  labs(title = "",
       x = "Method",
       colour = "Method",
       y = "Bias of Centrality Estimation (1st node)")
...
ggsave_rev("plot_power_combined.pdf", plot_power_combined, height = 16* 0.95, width = 16* 0.8,
       path = here::here("figures/"))
ggsave_rev("plot_power_combined.svg", plot_power_combined, height = 16* 0.95, width = 16* 0.8,
...
ggsave_rev("plot_power_bias_outstrength_presentation.pdf", 
...
ggsave_rev("plot_power_bias_strength.pdf", 
       ...
       path = here::here("figures/"))
ggsave_rev("plot_power_bias_strength.svg", 
       plot_power_bias_strength, height = 16* 0.95, width = 16* 0.8,
...
create_regression_plot <- function(df, 
  ...
  ggplot(data = df,
    ...
    ggh4x::facet_nested(n_id ~ outcome,
                        axes = "all",
                        remove_labels = "y") +
    scale_y_continuous(expand = c(0, 0), limits = c(-.45, .45)) +
    scale_color_manual(values = meth_colors) +
    theme_centrality() +
    theme(
...
Here, we investigate the variability of point estimates in specific simulation conditions, for which we need the raw results - this part is therefore not reproducible. Again, this code is commented out not to clash with our make execution. Simply uncomment and run to reproduce manually. 
```{r}
#| eval: !expr params$rerun
# if(isTRUE(use_rev)){
#   r3 <- readRDS(here::here("output/rev1_fixed_sigma/sim_full_rev1_cond23_fixed_sigma.rds-results_pc04669/results-row-2.rds"))
# } else {
...
#    geom_vline(colour = "darkgrey", xintercept = seq(1.5, 3, 1))
#  
# ggsave_rev("est_variability_plot.pdf", plot = est_variability_plot, 
#         height = 6, width = 9,
#         path = here::here("figures/"), device = "pdf")
#  
# # save plot as rds
# saveRDS(est_variability_plot, here::here("figures", "rev1", "est_variability_plot.rds"))
...
#| fig-width: 9
#| fig-height: 9
est_variability_plot <- readRDS(here::here("figures", "rev1", "est_variability_plot.rds"))

est_variability_plot
```
