...
In this script, we will conduct some additional simulations for the mlVAR model. It mostly adapts code from `centrality_simulation.qmd`.
...
sim_analyse <- function(condition, dat, fixed_objects = NULL){
  ...
  fit_mlvar <- tryCatch({suppressMessages(mlVAR::mlVAR(df_data,
                            ...
                            temporal = "correlated",
                            nCores = 1,
                            scale = TRUE))}, error = function(e) NA)
...
To note about data structure: - GVAR fit: already transposed into rows as lagged, cols as contemp - mlVAR fit: rows as lagged, cols as contemp - gimme fit: already transposed into rows as lagged, cols as contemp - bmlVAR fit: different data structure anyway
...
sim_summarise <- function(condition, results, fixed_objects = NULL){
  ...
  # should we use lm_beta to standardize?
  # needs to be set to true to work properly
  std <- TRUE
...
n_var <- 5
n_id <- 400
sim_pars$graph_sparse$beta <- sim_pars$graph_sparse$beta[1:5,1:5]
sim_pars$graph_sparse$pcor <- sim_pars$graph_sparse$pcor[1:5,1:5]
sim_pars$graph_sparse$kappa <- sim_pars$graph_sparse$kappa[1:5,1:5]
sim_pars$graph_nonsparse$beta <- sim_pars$graph_nonsparse$beta[1:5,1:5]
sim_pars$graph_nonsparse$pcor <- sim_pars$graph_nonsparse$pcor[1:5,1:5]
sim_pars$graph_nonsparse$kappa <- sim_pars$graph_nonsparse$kappa[1:5,1:5]
...
# started 2024-07-03 15:00

sim_results_mlvar <- SimDesign::runSimulation(
                                    design = df_design, 
                                    replications = n_rep, 
                                    ...
                                    ncores = n_rep,
                                    # debug = "analyse"
                                    filename = "mlvar-prelim-sim-synthdata-highn",
                                    # save_seeds = TRUE
                                    )
...
# Investigate Raw Fits

```{r}
res1 <- readRDS("scripts/mlvar-prelim-simulation-results_pc04798/results-row-1.rds")

res4 <- readRDS("scripts/mlvar-prelim-simulation-results_pc04798/results-row-4.rds")

```

## Random Effects

The random effects variance is actually okay. I previously thought that this may be one of the main reasons for performance issues, but that does not seem to be the case.

```{r}
# res1$results[[1]]$mlvar$fit_mlvar$beta

# Function to calculate element-wise standard deviation
elementwise_sd <- function(...) {
    sd(c(...))
}

# Calculate element-wise standard deviation using Map
result_sd <- do.call(Map, c(list(elementwise_sd), res1$results[[1]]$mlvar$fit_mlvar$beta))
result_sd <- do.call(Map, c(list(elementwise_sd), res4$results[[1]]$mlvar$fit_mlvar$beta))
result_sd <- do.call(Map, c(list(elementwise_sd), res4$results[[1]]$mlvar$fit_mlvar$pcor))

```

## Density

The standard deviation of densities is rather similar (0.0087) to the true sd of densities (0.011).

```{r}
sd(res4$results[[1]]$mlvar$dens_temp)
sd(unlist(res4$results[[1]]$true_cent$dens_temp))
```

## Centrality

What about network centrality measures? The SD looks better here

```{r}
sd(sapply(res1$results[[1]]$mlvar$outstrength, function(x) x[1]))
sd(sapply(res1$results[[1]]$true_cent$outstrength, function(x) x[1]))
...
```{r}
data.frame(
  est = sapply(res1$results[[4]]$mlvar$outstrength, function(x) x[1]),
  true = sapply(res1$results[[4]]$true_cent$outstrength, function(x) x[1])
) %>% 
  ggplot(aes(x = est, y = true)) + 
