...
graph_nonsparse <- readRDS(here::here("data/graph_nonsparse_koval.RDS"))

# sparse DGP
graph_sparse <- readRDS(here::here("data/graph_sparse_koval.RDS"))
```
...
```{r}
data.frame(
  est = sapply(res1$results[[1]]$gvar$outstrength, function(x) x[1]),
  true = sapply(res1$results[[1]]$true_cent$instrength, function(x) x[1])
) %>% 
