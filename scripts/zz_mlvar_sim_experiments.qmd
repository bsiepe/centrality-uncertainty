...
Try the function with synthetic data
```{r}
graph_nonsparse_synth <- readRDS(here("data/graph_nonsparse_synth.RDS"))
graph_sparse_synth <- readRDS(here("data/graph_sparse_synth.RDS"))
sim_data_loop <- sim_gvar_loop(
                     graph = graph_sparse_synth,
                     beta_sd = 0.1,
                     kappa_sd = 0.1,
                     n_person = 100,
                     n_time = 150,
...
graph_sparse_synth <- readRDS(here::here("data/graph_sparse_synth.RDS"))
graph_nonsparse_synth <- readRDS(here::here("data/graph_nonsparse_synth.RDS"))


# new synthetic graph
graph_nonsparse_synth_new <- readRDS(here::here("data/graph_nonsparse_synth_new.RDS"))
graph_sparse_synth_new <- readRDS(here::here("data/graph_sparse_synth_new.RDS"))
dgp_graph <- graph_nonsparse_synth_new

beta_sd_low <- 0.01
beta_sd_high <- 0.03
mean_diag_kappa <- mean(diag(dgp_graph$kappa)) 
kappa_sd_low <- 0.01 * mean_diag_kappa
kappa_sd_high <- 0.03 * mean_diag_kappa
sigma_sd_low <- 0.01
sigma_sd_high <- 0.03
n_id <- 200
n_tp <- 80
...
#                      change_density = TRUE,
#                      change_density_factors = c(0.6, 1.4))
...
# Simulate with simulateVAR function
ml_sim_low <- sim_gvar_loop(
                     graph = dgp_graph,
                     beta_sd = beta_sd_low,
                     kappa_sd = kappa_sd_low,
                     sigma_sd = sigma_sd_low,
                     n_person = n_id,
                     n_time = n_tp,
                     n_node = n_var,
                     sim_pkg = "mlVAR",
...
do.call(rbind, true_cent_low$outstrength) %>% 
  ...
  ggdist::stat_halfeye()+
  theme_centrality() +
  labs(title = "Difference between most central and second most central node",
...
```{r}
df_dens <- data.frame(dens_temp = do.call(rbind, true_cent_low$dens_temp),
                      dens_cont = do.call(rbind, true_cent_low$dens_cont)) %>% 
...
Check if the new density manipulation worked: 
```{r}
