...
graph_nonsparse <- readRDS(here("data","graph_nonsparse_synth.RDS"))
# sparse DGP
...
```{r}
lp <- log_posterior(mlvar_fit)
np <- nuts_params(mlvar_fit)
...
for(x in 1:length(pars)) {
  posterior <- as.array(mlvar_fit, pars[x])
...
bayesplot::mcmc_recover_intervals(
  true = rep(0,9),
  x = rstan::extract(object = mlvar_fit, c("reg_intercept_z")) %>% do.call(cbind, .) %>%
    as_draws_matrix()
)
